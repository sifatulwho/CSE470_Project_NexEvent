                ...
                <div class="p-6">
                    ...
                    <div class="space-y-4 mb-6" style="max-height: 500px; overflow-y: auto;">
                        @if($messages && $messages->count() > 0)
                                        ...
                                        <div class="max-w-xs lg:max-w-md">
                                            <div class="flex items-start space-x-2 {{ $message->sender_id === auth()->id() ? 'flex-row-reverse space-x-reverse' : '' }}">
                                                ...
                                                <div class="{{ $message->sender_id === auth()->id() ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-900' }} rounded-lg px-4 py-2">
                                                    <p class="text-xs font-semibold mb-1 opacity-70">{{ $message->sender->name }}</p>
                                                    <p class="text-sm">{{ $message->message }}</p>
                                                    <p class="text-xs mt-1 opacity-70">{{ $message->created_at->format('M d, H:i') }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            @endforeach
                        @else
                            <p class="text-gray-500 text-center py-4">No messages yet. Be the first to post!</p>
                        @endif
                    </div>

                    <!-- Message Form -->
                    <form method="POST" action="{{ route('events.messages.store', $event) }}" class="border-t pt-4">
                        @csrf
